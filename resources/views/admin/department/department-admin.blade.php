<x-admin-layout>
    ...
    <div id="departmentDetailModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-2xl w-full mx-4">
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">Detail Departemen</h3>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-900 dark:text-white" id="modalDesc"></p>
                    </div>
                </div>
            </div>
            <div class="mt-6">
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
            <thead class="bg-gray-100 dark:bg-gray-700">
                <tr>
                    ...
                    <th class="px-6 py-3 text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide border-b border-gray-300 dark:border-gray-600">Description</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @foreach ($departments as $department)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                        <td class="px-6 py-4 text-gray-900 dark:text-gray-300">{{ $department['id'] }}</td>
                        <td class="px-6 py-4 text-gray-900 dark:text-gray-300">{{ $department['name'] }}</td>
                        <td class="px-6 py-4 text-gray-900 dark:text-gray-300">{{ $department['desc'] }}</td>
                        <td class="px-6 py-4 flex space-x-2">
                            <button onclick="showModal('{{ $department['name'] }}', '{{ $department['desc'] }}')" 
    ...
    <script>
        function showModal(name, desc) {
            ...
            document.getElementById('departmentDetailModal').classList.remove('hidden');
            document.getElementById('departmentDetailModal').classList.add('flex');
        }

        function closeModal() {
            document.getElementById('departmentDetailModal').classList.remove('flex');
            document.getElementById('departmentDetailModal').classList.add('hidden');
        }
